...
plt.plot(
    df_uproot["size"],
    df_uproot["maxresident"],
    label="uproot-tensorflow",
...
for cs, rows in df_ROOT.groupby("chunksize"):
    label = labels[i]
    i += 1
    plt.plot(rows["size"], rows["maxresident"], label=label, linewidth=line_width)
...
plt.hist(
    df_uproot[(df_uproot["time"] < 0.01) & (df_uproot["time"] > 0)]["time"] * 1000,
    bins=100,
    label="TensorFlow",
    alpha=0.75,
)
plt.hist(
    df_ROOT[(df_ROOT["time"] < 0.01) & (df_ROOT["time"] > 0)]["time"] * 1000,
    bins=100,
    label="RBatchGenerator",
    alpha=0.75,
)
...
for cs, rows in df_ROOT.groupby("chunksize"):
    label = labels[i]
    i += 1
    plt.plot(rows["filesize"], rows["maxresident"], label=label, linewidth=line_width)

plt.xlabel("number of events", fontsize=axis_fontsize)
plt.ylabel("memory usage (b)", fontsize=axis_fontsize)
