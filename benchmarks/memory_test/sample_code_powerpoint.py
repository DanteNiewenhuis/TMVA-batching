from ROOT.TMVA.Experimental import (
    CreateTFDatasets,
    GetBatchGenerators,
...
gen_train, gen_validation = GetBatchGenerators(
    file_name, tree_name, chunk_rows, batch_rows, target="Type", validation_split=0.3
)

# Returns a TensorFlow Dataset
ds_train, ds_validation = CreateTFDatasets(
    file_name, tree_name, chunk_rows, batch_rows, target="Type", validation_split=0.3
)

# Returns a generator that returns PyTorch Tensors
gen_train, gen_validation = GetPyTorchDataLoaders(
    file_name, tree_name, chunk_rows, batch_rows, target="Type", validation_split=0.3
)
