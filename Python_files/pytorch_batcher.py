...
class PytorchGenerator:
    ...
    def load_chunk(self):
        ...
        data = torch.Tensor(data).view(self.chunk_rows, self.num_columns)
...
generator = PytorchGenerator(file_name, tree_name, columns, chunk_rows, batch_rows)
...
while True:
    ...
    dataloader = DataLoader(chunk, batch_size=batch_rows, shuffle=True)

    for batch in dataloader:
        times.append(time.time() - middle)

with open(f"{main_folder}results/Cpp_Python/{chunk_rows}_Pytorch.txt", "w") as f:
